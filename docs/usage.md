...
- `ロボット [2] の脱力する前の姿勢を覚える 角度 [-0.8, -103.5, 87.8, -106.4, -0.3, 1.7]`: stores the per-robot safe pose (6 joint angles in degrees) used by `安全に脱力する`. The `角度` slot takes any of: the `全関節角度を読む ロボット (n)` reporter dropped straight into it, a Snap! list, a JSON array string, or comma-separated numbers. An empty input clears the setting and restores the built-in default. A wrong count, or a `null` joint from an uncalibrated robot, raises a Japanese error naming the joint. The value is kept per robot in `localStorage`, so it survives a page reload.
