...
- `ロボット [1] の関節 [1] を [0.2] 秒動かす`: moves that joint at a constant 30 deg/s in the stored direction for the given time. Reads the joints once, then writes an interpolated target every 50 ms, so 0.1 s ≈ 3°, 1 s ≈ 30°. Clamped to 5 s per call and ±175°. Blocks the browser for the duration (synchronous XHR, like the other blocks).
