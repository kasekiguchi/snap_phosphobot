...
関節角度を読む 関節番号 [1]
全関節角度を読む
ロボット [1] の関節 [1] を [0] 度にする
ロボット [1] の全関節を 角度 [0, -90, 90, -90, 0, 0] にする
ロボット [1] の関節をまとめて動かす 指定 [2,-90  3,90]
PIDで関節 [1] を目標角度 [0] へ一歩うごかす dt秒 [0.1]
ホームに戻る
ロボット [1] をその場で止める
```

複数の関節をまとめて動かしたいときは、次の2つのどちらでも書けます。
`全関節を 角度 … にする` は空欄にした関節を今の角度のまま残すので、
`全関節角度を読む` の結果をそのままはめ込んで、一部だけ書き換える使い方もできます。
`関節をまとめて動かす 指定 …` は、動かしたい関節だけを「関節番号,角度」の組で並べます。
...
「力をゆっくり抜く」で移動する角度は、アームの個体やキャリブレーションによって適切な値が違います。
その値は次のブロックでロボットごとに設定できます（単位は度・6関節ぶん）。

```text
ロボット [2] の力を抜く前の姿勢を覚える 角度 [-0.8, -103.5, 87.8, -106.4, -0.3, 1.7]
```

決め方の手順（`全関節角度を読む` をはめ込む方法・おすすめ）:

1. `力をゆっくり抜く` でトルクをOFFにするか、手でアームを安全な姿勢（机に置いても倒れない低い姿勢）に動かす
2. `全関節角度を読む ロボット [1]` ブロックを、`力を抜く前の姿勢を覚える` の **`角度` の入力欄にそのままドラッグしてはめ込む**

   ```text
   ロボット [1] の力を抜く前の姿勢を覚える 角度 (全関節角度を読む ロボット [1])
   ```

3. その姿勢のまま、はめ込んだブロックを実行する（今の姿勢が安全角度として保存される）
4. 以降、`力をゆっくり抜く` はその角度を使う
...
- 秒数を大きく（例 `0.6`）→ 目標を行き過ぎて大きく振動する
- 秒数を小さく（例 `0.1`）→ 振動は小さいが、目標に届くまでが遅い

初期値の `0.4` は、振動しているのが教室からはっきり見える大きさです
（シミュレーションでの実測: `0.1`秒→±1.6度、`0.2`秒→±3.2度、`0.4`秒→±6.2度、`0.6`秒→±9.2度）。
...
3. 再度 `robot/config` で `config` が入っていれば OK。Snap! の「ロボット 2 の関節 1 を 0 度にする」を再実行する。
