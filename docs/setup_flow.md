...
```text
ロボットの接続先を [http://<Pi5のIPアドレス>:8021] にする
```
...
`ロボット (n) の全関節の角度` の結果をそのままはめ込んで、一部だけ書き換える使い方もできます。
...
```text
ロボット [2] の脱力する前の姿勢を覚える 角度 [-0.8, -103.5, 87.8, -106.4, -0.3, 1.7]
```

決め方の手順（`ロボット (n) の全関節の角度` をはめ込む方法・おすすめ）:

1. `安全に脱力する` でトルクをOFFにするか、手でアームを安全な姿勢（机に置いても倒れない低い姿勢）に動かす
2. `ロボット [1] の全関節の角度` ブロックを、`脱力する前の姿勢を覚える` の **`角度` の入力欄にそのままドラッグしてはめ込む**

   ```text
   ロボット [1] の脱力する前の姿勢を覚える 角度 (ロボット [1] の全関節の角度)
   ```
...
`ロボット (n) の全関節の角度` の出力（JSON配列文字列）もそのまま受け付けます。
...
```text
ロボット [1] の脱力する前の姿勢を覚える 角度 (ロボット [1] の全関節の角度)
ロボット [2] の脱力する前の姿勢を覚える 角度 (ロボット [2] の全関節の角度)
```
