...
> **シミュレーションのロボットは1台だけ**です。ブロックの `ロボット` は必ず **1** にしてください
> （`robot_id=0` に対応）。ロボット2 を指定するとエラーになります。
...
目標角度から離れたところ（例: `目標角度にうごかす ロボット [1] 関節 [1] 角度 [-40]`）から始めると、
目標に近づいたあと **±4.5度くらいで振動し続けて止まらない** のが見えます。
...
| `秒動かす` の値 | 見えること |
|---|---|
| `0.5` | 目標を大きく行き過ぎて激しく振動する |
| `0.2` | ±4〜5度で振動が続く |
| `0.02` | 振動は小さいが、目標に届くまでがとても遅い |
...
接続確認・読み取り・`目標角度にうごかす`・`回転方向`＋`秒動かす`・ON-OFF ループ・脱力・PID を順に流し、
